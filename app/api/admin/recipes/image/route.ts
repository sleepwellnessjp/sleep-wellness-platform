...
function isAllowedMime(type: string): boolean {
  ...
  return (RECIPE_IMAGE_MIME_TYPES as readonly string[]).includes(normalized);
}
...
export async function POST(request: Request) {
  try {
    ...
    if (file.size > RECIPE_IMAGE_MAX_BYTES) {
      return NextResponse.json(
        {
          error: `画像が大きすぎます（上限 ${Math.round(RECIPE_IMAGE_MAX_BYTES / (1024 * 1024))}MB）`,
        },
        { status: 400 },
      );
    }
    ...
    console.error("[api/admin/recipes/image POST]", error);
    return NextResponse.json(
      {
        error:
