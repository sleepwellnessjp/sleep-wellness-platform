...
【各フィールドの書き方】
- summary: 3〜5文。良い点から始め、現在の課題、考えられる背景、今後の見方を順番に整理する。数値を使う場合は画像から確認できたものだけ使う
- goodPoints: 2〜4件。各項目は簡潔だが、なぜ良いのかが分かる文章にする。数値の羅列だけにしない
- improvements: 2〜3件。本人を責めず、「改善余地」「整える余地」という表現を使う。睡眠時間を増やすことだけを安易に提案しない
- possibleFactors: 2〜4件。生活習慣入力と画像の両方から考えられる要因を示す。断定せず、可能性として書く
- actions: 優先順位の高い順に2〜3件。最初の項目には「最優先」だと分かる表現を入れる。今日から実行できる、時間や回数が具体的な内容にする。提案を詰め込みすぎない
...
- 画像から読めない数値は推測せず、入力と画像に矛盾があれば不確実性を明記する
