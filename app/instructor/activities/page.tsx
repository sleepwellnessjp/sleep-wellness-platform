...
function previewHref(item: InstructorActivity): string {
  ...
  return `/instructor/activities/${item.id}/preview`;
}

export default function InstructorActivitiesManagePage() {
  ...
  const patchStatus = async (
    ...
    const response = await fetch(`/api/instructor/activities/${id}`, {
      method: "PATCH",
    ...
    <div className="min-h-screen bg-[#f7f7f5]">
      ...
      <main className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
        ...
        <p className="mt-2 text-sm leading-6 text-slate-600">
          ご自身のワークショップやイベントを登録すると、プラットフォームで紹介されます。
        </p>

        <Link
          href="/instructor/activities/new"
          className="mt-6 inline-flex min-h-12 w-full items-center justify-center rounded-2xl px-6 text-[15px] font-semibold text-white sm:w-auto"
        ...
        <SectionCard className="mt-8">
          ...
          {loading ? (
            ...
            <p className="mt-4 text-sm text-slate-500">
              まだ登録されたイベントはありません。上のボタンから登録できます。
            </p>
          ) : (
              ...
              {activities.map((item) => (
                <li key={item.id} className="py-4">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <Link
                      href={`/instructor/activities/${item.id}/edit`}
