...
function nightMovementCard(
  ...
  return {
    ...
    title: "今夜の動き",
    philosophy:
      "メラトニンヨガ™は、昼の間のヨガ™から立位を減らし、身体を休息モードへ戻すための夜の流れである。立位は意識を内側に向かせるものだけを残す。",
    steps: [
      `メラトニンヨガ™（夜）を ${sequence.durationHint} で行う。`,
      `順序：${blockLine}`,
      "吸3：吐6（きつければ5）の呼吸をしながらゆっくり動く。",
      "アーサナのキープ中は吸3：吐6（きつければ5）の呼吸を1〜3回する。",
      ...extra,
      "呼吸法は、上の『今夜の呼吸』のとおりに行う。",
    ],
    dosageBadges: [
      sequence.nameJa,
      sequence.durationHint,
      "意識を内側に向かせる立位のみ",
      "キープ中は吸3：吐6を1〜3回",
    ],
    cautions: [
...
function dayPracticeCard(
  ...
  return {
    ...
    steps: [
      ...
      `立位は順序を厳守する: ${standing}。`,
      "呼吸法：スーリヤベーダナ（またはウジャイ）。続いて自律訓練法（日中は消去動作を必ず行う）。最後に呼気延長 吸3：吐6（きつければ5）。",
