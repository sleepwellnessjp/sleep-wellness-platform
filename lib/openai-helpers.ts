...
- sleep_overview … 睡眠概要（睡眠スコア中心）。ホームが無いときのみ睡眠スコアの正。全就床時間が見えれば返す
- sleep_detail … 睡眠詳細（時間・全就床・効率・負債・潜時・入眠・起床）
...
- sleep_overview → 睡眠スコア / 睡眠時間 / 全就床時間（ホームが無いときの睡眠スコア正）
  ※「全就床時間」は睡眠時間と別エントリ。見出し「全就床時間」の主値のみ（下段比較値は捨てる）
- sleep_detail → 睡眠時間 / 全就床時間 / 入眠時間 / 起床時間 / 睡眠効率 / 睡眠負債 / 入眠潜時 / 体内時計
  ※睡眠スコアは返さない（ホーム／概要を正とする）
  ※「全就床時間」は必須（見えるとき）。睡眠時間・必要睡眠・入眠潜時と混同しない
- bed_wake → 入眠時間 / 起床時間 / 入眠潜時
- sleep_stages → 覚醒・レム・浅い・深いの4行すべて必須（各行の時間と%を別エントリ。「浅い睡眠」を省略しない）/ ノンレム見出しがあればその時間と% / SpO₂ / 呼吸速度
...
==============================
【最優先・見逃し禁止】入眠・起床・全就床時間・皮膚温度・ストレス
==============================
...
【全就床時間】
- ラベル例: 全就床時間 / 全就床 / Time in Bed / ベッド滞在時間
- 値は H:MM または N時間M分（例 5:47 / 5時間47分）。必ず label「全就床時間」で返す。
- 睡眠時間・必要睡眠時間・入眠潜時・就寝時刻と別エントリ。混同禁止。
- カード下段の小さな比較値（昨日差）は捨て、大きな主値だけを返す。
- sleep_detail / sleep_overview（睡眠トレンド）に見えるときは省略しない。
