...
--
-- clients は最小情報のみ（id / owner_id / name / name_kana / memo / created_at）。
...
begin
  ...
  insert into public.clients (owner_id, name, name_kana, memo)
  values (v_uid, v_name, v_kana, v_memo)
...
comment on function public.create_client_with_profile(text, text, text) is
  'clients（最小カラム）と空の client_profiles を同一トランザクションで作成。失敗時はロールバック。';
...
grant execute on function public.create_client_with_profile(text, text, text) to authenticated;
